...
import BlockProfile from "../profile/block.model";
...
const getPostService_v2 = async (req: Request) => {
  const userIdStr = req.user?._id?.toString() || null;
  const userRole = req.user?.role || null;

  console.log("Get post id:", userIdStr);

  const blocked = userIdStr
    ? await BlockProfile.find({ blockerId: req.user._id }).select("blockedId")
    : [];
