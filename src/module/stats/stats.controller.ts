...
const scanning: RequestHandler = catchAsync(async (req, res) => {
  ...
  console.log("scanning file:", scan);
  ...
  const matchedProducts = await Product.aggregate([
    ...
    },
    {
      $lookup: {
        ...
        localField: "brandId",
        foreignField: "_id",
        pipeline: [{ $project: { _id: 0, brandName: 1 } }],
        as: "brandData",
      },
    },
    {
      $addFields: {
        brandName: {
          $ifNull: [{ $arrayElemAt: ["$brandData.brandName", 0] }, null],
        },
      },
    },
    { $unset: "brandData" },
...
//   }
  ...
  const result = await StatsServices.fetchAggregationTwo<IBrand>(
    Brand,
    ["brandName", "brandLogo", "theme"],
    req.query,
  );
...
const appFirstStatsTwo: RequestHandler = catchAsync(async (req, res) => {
  ...
  const result = await StatsServices.fetchAggregation<IBrand>(
    Brand,
    ["brandName", "brandLogo", "theme"],
    req.query,
  );
...
const getMonthlyProductOrders: RequestHandler = catchAsync(async (req, res) => {
  ...
  const yearStr = Array.isArray(yearParam)
    ? (yearParam[0] as string)
    : (yearParam as string);
  ...
  const pipeline: PipelineStage[] = [
    ...
    // 7. Match by brandId and year (using order.createdAt)
    {
      $match: {
        ...
        $expr: {
          $eq: [{ $year: "$createdAt" }, currentYearNum],
        },
      },
    },
...
const getBrandStats: RequestHandler = catchAsync(async (req, res) => {
  ...
  if (productIds.length > 0) {
    ...
    const cartProductIds: Types.ObjectId[] =
      cartResult[0]?.cartProductIds || [];
